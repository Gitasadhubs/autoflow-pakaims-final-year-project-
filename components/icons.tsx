...
export const GitHubIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg height="32" aria-hidden="true" viewBox="0 0 16 16" version="1.1" width="32" data-view-component="true" className={className}>
...
export const Spinner: React.FC<{ className?: string; title?: string }> = ({ className, title }) => (
...
export const StatusIcon: React.FC<{ status: 'queued' | 'in_progress' | 'completed' | null, conclusion: string | null, className?: string }> = ({ status, conclusion, className = 'h-5 w-5' }) => {
    if (status === 'queued') {
        return <div className={`rounded-full bg-yellow-500 ${className}`} title="Queued"></div>;
    }
    if (status === 'in_progress') {
        return <Spinner className={`${className} text-blue-500`} title="In Progress" />;
    }
    if (status === 'completed') {
        ...
        if (conclusion === 'failure' || conclusion === 'cancelled' || conclusion === 'timed_out') {
         ...
         // Handle other conclusions like skipped, neutral
        return (
            <svg className={`${className} text-gray-500`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <title>Completed: {conclusion}</title>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
        );
    }
    
    // If status is null, there are no runs.
    if (status === null) {
      return (
        <svg className={`${className} text-gray-400`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <title>No recent runs</title>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      );
    }
    
    return <div className={`rounded-full bg-gray-500 ${className}`} title="Unknown"></div>;
};
...
export const RocketIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15.59 14.37a6 6 0 01-5.84 7.38v-4.82m5.84-2.56v4.82a6 6 0 01-1.844 4.14m-3.996-11.54a6 6 0 015.84 2.56m-5.84-2.56a6 6 0 01-5.84 2.56m5.84-2.56v-4.82a6 6 0 015.84-7.38m-5.84 7.38v-4.82a6 6 0 00-5.84-7.38m5.84 7.38l-4.14 1.844m-4.14-1.844l4.14 1.844m-4.14-1.844a6 6 0 005.84 7.38m-5.84-7.38l-5.84 2.56" />
    </svg>
);
...
export const RefreshIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0011.664 0l3.181-3.183m-11.664 0a8.25 8.25 0 010-11.664l3.181 3.183m0 0l-3.181-3.183" />
  </svg>
);
